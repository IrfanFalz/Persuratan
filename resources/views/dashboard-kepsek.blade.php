        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center space-x-2 sm:space-x-4">
                    ...
                    @php
                        $sessName = session('name') ?? optional(Auth::user())->nama ?? 'User';
                        $sessRoleKey = session('role') ?? strtoupper(optional(Auth::user())->role ?? '');
                    @endphp

                    <!-- User Info - Hidden on mobile -->
                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-medium text-gray-800">{{ $sessName }}</p>
                        <p class="text-xs text-gray-600">{{ $roles[$sessRoleKey] ?? $sessRoleKey }}</p>
                    </div>
                    ...
                    <div class="space-y-4">
                        @foreach($pending_approval as $request)
                            <div class="p-4 sm:p-6 bg-gray-50 rounded-lg border border-gray-200 hover:border-orange-300 transition duration-200 cursor-pointer"
                                 onclick="openDetailModal(@json($request))">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="mt-2">
                                            <button type="button"
                                                    ...
                                                    onclick="event.stopPropagation(); openDetailModal(@json($request))">
                            ...
                            <div class="flex items-center space-x-3 p-3 sm:p-4 bg-gray-50 rounded-lg">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="font-medium text-gray-800 text-sm sm:text-base truncate">
                                        {{ ucfirst($activity['status']) }}: {{ $activity['type'] }} - {{ $activity['teacher'] }}
                                    </p>
                                    <p class="text-xs sm:text-sm text-gray-600">
                                        {{ date('d/m/Y H:i', strtotime($activity['date'])) }} ({{ $activity['duration'] }})
                                    </p>
                                </div>
                            </div>
                        @endforeach
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('dashboard.kepsek.approval') }}">
                    ...
                    <input type="hidden" name="id_persetujuan" id="approvalRequestId">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('dashboard.kepsek.approval') }}">
                    ...
                    <input type="hidden" name="id_persetujuan" id="rejectionRequestId">
