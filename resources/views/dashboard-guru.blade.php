            ...
            <div class="lg:col-span-1 card">
                ...
                <div class="space-y-3 sm:space-y-4">
                    <div class="flex items-center justify-between p-3 sm:p-4 bg-green-50 rounded-lg">
                        ...
                        <span class="bg-green-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold">{{ $approvedCount }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 sm:p-4 bg-yellow-50 rounded-lg">
                        ...
                        <span class="bg-yellow-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold">{{ $pendingCount }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 sm:p-4 bg-blue-50 rounded-lg">
                        ...
                        <span class="bg-blue-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold">{{ $totalCount }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-3 sm:space-y-4">
                @forelse($notifikasi as $notif)
                    <div class="notification 
                        {{ $notif->status === 'disetujui' ? 'bg-green-50 border-green-500' : ($notif->status === 'ditolak' ? 'bg-red-50 border-red-500' : 'bg-blue-50 border-blue-500') }}">
                        <div class="flex items-start">
                            <i class="fas 
                                {{ $notif->status === 'disetujui' ? 'fa-check-circle text-green-500' : ($notif->status === 'ditolak' ? 'fa-times-circle text-red-500' : 'fa-file-alt text-blue-500') }} 
                                mr-3 mt-1 flex-shrink-0"></i>
                            <div class="min-w-0 flex-1">
                                <p class="font-medium text-gray-800 text-sm sm:text-base">{{ $notif->pesan }}</p>
                                <p class="text-xs sm:text-sm text-gray-600">{{ $notif->created_at->format('d M Y, H:i') }}</p>
                            </div>
                        </div>
                    </div>
                @empty
